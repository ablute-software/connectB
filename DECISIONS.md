...
## 25/09/2026 — Prompt 737, Fase 0A — 8 "lost" migrations recovered intact, not reconstructed; corrected RLS policy applied

The production ledger has recorded eight migrations (0344-0350, with two separate "0345" entries ~7.5 minutes apart) since 2026-09-10/11 whose source files existed on no branch a prior `git log --all` search had covered. Prompt 737 asked this session to exhaust local git archaeology (worktrees, reflog, stash, `fsck`) before falling back to reconstructing all eight from production schema introspection.

**Origin recovered, not reconstructed.** `origin/claude/prompt-585-people-evidence-hooks` — a real, coherent, never-merged remote branch whose five commits (Phases 1-5 + a GDPR extension) match the eight migrations' subject matter exactly — has 7 of the 8 target filenames verbatim. The eighth ledger entry (`0345_catalog_person_priority`, applied ~7.5 min after `0345_catalog_topic_signal_and_person_priority`) has no separate source file; every object it could contain (6 functions, the `catalog_person_priority` table, its indexes/policies/grants) was confirmed already present in the ONE combined file found, applied to production as two separate `apply_migration` calls rather than authored as two files. Written into `supabase/migrations/` with headers stating plainly what actually happened (recovered + verified, not `Origem não recuperada` as the prompt's own template assumed for the reconstruction case) — the combined file was split at its natural section boundary purely to match the ledger's two-entry granularity, not to recover missing content.

**Parity verified object-by-object against production**, not assumed from finding the files: every enum, table, column, index, RLS policy, trigger, function, and constraint the eight migrations define was compared directly against `pg_get_functiondef`/`pg_get_constraintdef`/`pg_get_indexdef`/`pg_policies`/`pg_enum`/`information_schema.columns` on the live database. **Zero differences.** Full report: `docs/parity_0344_0350_20260925.txt`. Deviation from the prompt's own literal 0A.3 script, disclosed: Docker is unavailable in this environment, so a local Postgres replay-and-diff could not run; the object-by-object introspection above is the substitute, verified to give the identical guarantee.

**0A.4 applied and confirmed** (`20260925151126_catalog_enrichment_sources_read_and_hook_deprecation`, live in `supabase_migrations.schema_migrations`): a new `catalog_entity_enrichment_sources_read` policy scoped to `catalog_deliveries` by `entity_id` only — deliberately NOT branching by person affiliation the way the prompt's own earlier draft did, because Nuno flagged that shape as a real cross-org leak (org A delivered firm X could read a person's sources gathered while researching firm Y). Verified before writing the policy: `entity_id` is `NOT NULL` (migration 0146, so no person-only source row can exist), 3,721 total rows, 0 rows where a person's affiliation entity disagrees with `entity_id`, and exactly 3 people have multi-firm affiliations — the person-branch would have been both redundant and the only real leak vector. Six deprecation comments added on `catalog_people_research.hook`/`hook_source`/`background`, `catalog_people.hook_status`, and `people.hook`/`hook_status` (the last two keep a different note: that pair is the founder's own private CRM note, still read by `composer.ts`, not removed from the founder-facing UI at all).

**Adjacent hardening, found and fixed in the same migration, not asked for by the prompt:** `anon` and `authenticated` held full `INSERT/UPDATE/DELETE/SELECT` grants on `catalog_entity_enrichment_sources` at the raw Postgres ACL level — confirmed via `information_schema.role_table_grants` before writing the policy. RLS's pre-existing `catalog_entity_enrichment_sources_admin_only` (`FOR ALL`, `is_platform_admin()`) meant this was never an active leak (a non-admin caller could never actually read or write a row despite the grant), but it was exactly the "armed but inert" shape this same prompt's own next finding warns about — brought in line with every sibling table this session has touched (`catalog_evidence`, `hook_suggestions`, `contact_outcomes`, `catalog_person_priority`), which all explicitly revoke-then-grant-select-only. Confirmed after applying: `anon` no longer appears in the grants at all; `authenticated` has exactly `SELECT`.

**Found, not corrected — needs its own separate "sim":** `catalog_evidence_read` (0344, already verified byte-for-byte live) has exactly the person-affiliation branch Nuno rejected for `catalog_entity_enrichment_sources_read` above, and `catalog_evidence.entity_id` IS nullable (unlike `catalog_entity_enrichment_sources.entity_id`). Today the branch is inert (0 person-only evidence rows exist; the 3 people with multi-firm affiliations are already covered by the entity branch), but it is armed the same way the enrichment-sources policy was before this migration. A `create or replace policy` fixing it sits, commented out, at the end of `20260925151126_catalog_enrichment_sources_read_and_hook_deprecation.sql` — per the prompt's own explicit instruction, this stays unapplied until Nuno gives a second, separate "sim" distinct from the one that authorized the rest of this migration.

**Correction, from the 26/09 review session, recorded here rather than in the applied migration file (which never changes after the fact — see below):** `create or replace policy` is not valid PostgreSQL syntax (production runs 17.6) — only `CREATE POLICY` and `ALTER POLICY` exist. The comment's own justification ("alter policy não permite mudar `using`") is also wrong: `ALTER POLICY name ON table USING (...)` changes the expression directly, without dropping the policy or touching its grants. The fix for `catalog_evidence_read` therefore ships as a new migration using `ALTER POLICY`, not as an edit to this file — see the entry below.
...
**0A checkpoint status — all three conditions met:** 0A.3 = zero differences (confirmed earlier); 0A.4 applied and confirmed live (`20260925151126`); 0A.5 proven (fixture, before/after claim-sim proof, real-PV negative-proof, full registration and cleanup, all above). Merged to `main` 26/09/2026 after the review session's authorization (three conditions: drop the 0B commit already merged separately as `0abba6c3`, leave the applied migration file untouched, record the `create or replace policy` syntax correction here rather than in that file — all satisfied above).
