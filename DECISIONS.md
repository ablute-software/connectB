...
**Prompt 718 (2026-09-22) — gap-assist/Review/Blueprint interrogation flow: a real, reconstructed production bug (key instability), not a theoretical one.** Parts A, B, C, D, F, G, H, reconstructed from Nuno's own real production usage on Sherlock Deal (`48a7c481-…`) the same day.

**Part A — `gapKey` stability.** `company-gaps.ts`'s `gapKey` fell through to `relatedClaimIds[0]` for G1/G3/G6 — and `relatedClaimIds` for those three rules is the WHOLE category (`ruleG1`/`ruleG3`/`ruleG6` map every claim in `tracao_gtm`/`equipa`/`funding`+`ask`), not one claim about this specific gap. The key changed from `G1:` to `G1:<first-claim-id>` the instant the first claim in the category existed — remounting `GapInterrogation` (`key={gap.key}`), dropping `answeredRules`' exact match against the OLD key stored in `source_ref`, and making a just-saved answer reappear as new. Fix: `STABLE_KEY_RULES = {G1,G3,G6}` — the discriminant is now permanently empty for these three; every other rule (G2/G3b/G3c/G4/G5/G8) is unchanged. `isGapAnswered` does exact match first (always correct going forward), with a rule-prefix fallback used ONLY for these three stable rules (never for a discriminated rule like G3b — confirmed by a dedicated negative test — a prefix match there would wrongly close a different founder's own still-open gap) to keep recognizing an answer recorded before this fix.

**Part B — "ask once, then inform, never re-ask."** `/api/blueprint` GET now returns `gaps` (unanswered) and a new `standing` array: a `STANDING_RULES` (`G1`/`G6` only — the two rules `checkStillOpen` already treated as structurally re-firing) gap that is BOTH still firing AND already answered becomes a stated fact (`rule`/`message`/`why`/`closesWhen`/the founder's own cited `answer`/`answeredAt`), never a re-asked blank question. `KnowledgeHealthPanel` renders it with one "Update answer" link; both `BlueprintPanel.tsx` and `ReviewPanel.tsx` wire that link to reopen the gap's own `GapView` for a fresh answer (Part C's replace logic handles the write). The OLD in-memory `stillOpenInfo`/"Got it, hide for now" mechanism in `GapInterrogation.tsx` — whose `dismissed_explicit` write went to a `gap_questions` table `/api/blueprint` GET never actually read back — is deleted entirely, along with `answer/route.ts`'s now-redundant `checkStillOpen`/`STILL_OPEN_CLOSES_WHEN` (the `closesWhen` text moved to `company-gaps.ts`, shared by the GET route's own `standing` builder). "Skip this one" (rotation-only, unaffected) still works exactly as before.

**Part C — replace vs. amend, and a bare "no" is never enough to write anything.** Two independent fixes: (1) `isTrivialFreeText` — free text under 3 words or a bare yes/no/sim/não, with no other informative chip also chosen, is routed exactly like a non-informative chip (`dismiss`, with a `note` explaining why) — this is what actually prevents the real corruption below from recurring. (2) When the free-text target claim IS the founder's own prior answer to this EXACT gap (`source_ref === gap:${gapKey}`, exact match — not a rule-only prefix, which would wrongly merge a different founder's/field's own answer for G3b/G8), the write is a REPLACE, never the old blind `${old} ${new}`.trim()` append; `routeFreeTextAnswer` (the AI amend-vs-new-claim call) now only ever runs for a target that is NOT that same-rule founder answer. Confirmed against the real corrupted claim `a49104fc-666f-…` (`"...such as accelerators. no"` — a bare "no" free-text follow-up got AI-amended onto an unrelated claim in the same whole-category `relatedClaimIds`) and the real duplicate claims `31a3d440-…`/`2cbf5aef-…` (G3/G6, created because the volatile key made a re-answer look like a brand-new question): all three exist in production exactly as this prompt described, confirmed by direct SQL on 2026-09-22, no drift. **Repair SQL proposed, not applied** — `docs/prompt-718-part-c-data-repair-proposal.md` — waiting on Nuno's explicit OK before running.

**Part D — the G3 draft timeout, and 60s if the plan allows.** Root cause confirmed in code (not just inferred): `selectTeamDocumentCandidates`'s final fallback (no `team_governance` folder, no name match) attached EVERY clean PDF in the org with only a document-COUNT cap (3) — a 40-page deck passed the same cap as a 1-page CV — plus two SEQUENTIAL 4s LinkedIn fetches (worst case ~8s), inside `maxDuration=30`. Fixed: `rankTeamDocuments`/`selectWithinPageBudget` (`gap-assist-sources.ts`) rank every clean PDF by team_governance-folder → name-match → rest, then fewest KNOWN pages first (an unextracted document's page count is unknown, so it "counts as the cap" — it can only ever be the one document attached, never combined with anything else), and keep adding while the running total stays within `MAX_ATTACHED_PDF_PAGES = 15`; LinkedIn fetches now run in `Promise.all` with a single shared 3s budget instead of 2×4s sequential. `maxDuration` raised to 60 (the real Vercel Hobby-plan ceiling per CLAUDE.md's own cron-limits note — reported here since actual production behavior at that ceiling couldn't be confirmed live). The client (`GapInterrogation.tsx`) now shows "This took too long to answer — try again in a moment." on any non-JSON response (a platform-level timeout), never the old generic "AI assist failed." — the route itself already always returned real JSON on a thrown error; the gap was specifically the timeout case, which the client couldn't tell apart from a real failure before this. Same budget function is written generically so Prompt 711's own G6 (round-document) attachment path can reuse it directly when that branch merges (Part E, not done in this pass).

**Part F — a draft's context, not just accepted claims.** `knowledgeToAtoms(readKnowledgeSources(admin, orgId))` — the exact same ingestion source `/api/blueprint`'s POST already uses, no new source, no migration — is now ALSO read in-memory for a gap-assist draft and labeled in the prompt as "From your profile and settings (not yet confirmed as a claim)", closing the real gap this org hit: `orgs.round_use_of_funds`/`round_target_eur`/`round_instruments` were populated, but a claim only gets created by running a Blueprint analysis (0 rows in `blueprint_analyses` for this org), so a draft could never see them. The "AI drafted this from…" sentence now lists only `sourcesRead: string[]` — what THIS call actually read (`accepted claims`, `your profile and settings`, `team profiles`, `N Vault document(s)`, `LinkedIn (N profile(s))`), never a static claim about what the feature theoretically could read. The insufficient-answer message states what was checked and a concrete next step ("Add it in Settings → Team, or upload a CV/bio in the Vault, then try again.") instead of a flat "fill it in yourself."

**Part G — multi-select for G1/G3/G6.** `GapInterrogation.tsx`'s chip state is now `string[]` for every rule (single-select rules just never accumulate more than one). `routeAnswerMulti` (`company-gaps.ts`) delegates straight to the existing `routeAnswer` when 0–1 options are chosen (every non-multi-select rule's behavior is byte-identical), and only for 2+ chosen chips applies "dismisses only if ALL chosen chips are non-informative" — one real chip among several still means real information was given. Statement-building joins chips with `', '`.

**Part H — wallet usage visible on this screen.** `GapInterrogation.tsx` fetches `fetchWalletStatus('blueprint_gap_draft' | 'blueprint_gap_polish')` (whichever the button would actually charge, based on whether the founder has typed anything) and shows "`N` AI credit(s) left this month" next to the AI buttons. Whether this becomes visible everywhere else in the app stays Nuno's own decision, out of scope here.

**What was NOT done in this pass:** Part E (merging `claude/prompt-711-gap-assist-vault-g6`, and applying Part D's page-budget fix to its own G6 attachment path) — deferred to a follow-up, since 711 predates this branch and needs its own rebase; the data-repair SQL above, pending Nuno's OK; Prompt 719/720's own separate, unrelated findings (queued, not part of this prompt).

**Verification.** tsc: EXIT=0. eslint (worktree-safe `--no-eslintrc` form): 0 errors, 0 warnings across all 11 changed/new files. vitest: 4057/4058 (the same one pre-existing, unrelated `market-facts-view.test.ts` locale flake every prompt in this session has seen) — 26 new tests across `company-gaps.test.ts` (gapKey stability, isGapAnswered exact+prefix+negative-leak cases, `routeAnswerMulti`, `isTrivialFreeText`, `closesWhenText`/`STANDING_RULES`) and `gap-assist-sources.test.ts` (`rankTeamDocuments` tiering/fewest-pages-first/unknown-sorts-last, `selectWithinPageBudget` budget enforcement and the "unknown counts as the cap" rule). Build: EXIT=1 on the first attempt (the same `next/font` Google Fonts transient-network failure this session has seen before, e.g. Prompt 715/716's own builds), EXIT=0 on immediate retry. Not live-browser-tested — no demo-mode path exercises the gap-assist AI calls (they'd spend real wallet credits against a real org), so the multi-select UI and the "standing fact" card are confirmed by direct code reading and the component's own logic, not a click-through.

**Status:** pushed to `claude/prompt-718-gap-assist-fixes`, not merged — holding for Nuno's explicit go-ahead, and for the data-repair SQL's own separate OK.
