...
10. **Commitments page (Prompt 603) — built, switched off until legal review.** `/welcome/commitments` and its acceptance record (`terms_acceptances`, version `commitments-1.0`) work today; the automatic post-signup redirect is behind `COMMITMENTS_GATE_ENABLED=1` (Vercel env) because the page creates contractual commitments and the prompt itself requires a lawyer's pass first. Three answers gate the switch: (1) does the AI provider train on submitted data — the promise was REMOVED from the structure in Prompt 608 (604 §A: "sai da estrutura, não fica como espaço vazio"), so there is no dormant line and no flag; if the contract ever confirms it, the promise is written then, as a new version; (2) hosting — measured: Supabase `eu-west-2` (London), Vercel functions `iad1` (US), Stripe/Anthropic/Resend US-based, so transfers outside the EEA exist and the SCC wording matters; (3) the controller — the Terms (v3) name Exotictarget, Lda; confirm it is the same entity for the Privacy Policy. `/legal/subprocessors` carries a "draft under legal review" banner until then.
