...
9. **Platform badges, Phase 2 (Prompt 601, specified, not built).** Phase 1 shipped 2026-09-07 (`platform_badges`, migration 0337): tech master and pioneer granted by hand from Startups → Badges, shown on Plans & Billing and the about page, free/25% reaching the invoice through Stripe coupons, the 2-month window with 75/90/98% warnings and the "Tech master lapses" queue, and a bounded, logged, mark-less rank boost in the MatchDeal deck (`platform_badge_rank_params()`: 2 per deck). Three questions close before the computed badges (Sedulous, Dyed-in-the-wool, Toughness) are built: (a) what counts as "accessed" — proposed: a `usage_sessions` row with real activity (Phase 1 already uses ≥60 s), days in Europe/Lisbon; (b) sliding window evaluated daily — assumed; (c) once earned, kept forever — assumed, by coherence with Sedulous. Also pending: the first real tech master grant (none exists — the only row is a revoked verification fixture on a test org), and a live check of the Stripe coupon path once any org holds a subscription (0 today).

10. **Commitments page (Prompts 603/604/606/608) — built, switched off until legal review.** `/welcome/commitments`, reachable any time from Help & support (604 §A's own recommendation, taken 2026-09-12). Prompt 604 corrected the register: this is advertising ("é apenas publicidade disfarçada"), not a second contract, so it no longer writes to `terms_acceptances` — that reused the real Terms' own table, and was a live (never-triggered) bug: `/api/terms/status` reads that table's latest row for the user regardless of version, so a commitments row would have shadowed a founder's real Terms acceptance. Registration is now one boolean, `org_members.commitments_seen` (migration `20260912140000`), applied to production 2026-09-12; zero founders were ever affected (the gate has been off since 603 shipped, so zero commitments-versioned rows were ever written). The nine promises are 604's literal marketing-voice text, numbered 1-9 with no gap (the gap 608 kept for the AI-training removal protected a versioned acceptance record that no longer exists). The automatic post-signup redirect stays behind `COMMITMENTS_GATE_ENABLED=1` (Vercel env) because the page still needs a lawyer's pass. Three answers gate the switch: (1) does the AI provider train on submitted data — the promise stays REMOVED from the structure (no dormant line, no flag); if the contract ever confirms it, the promise is written then, as new text; (2) hosting — measured: Supabase `eu-west-2` (London), Vercel functions `iad1` (US), Stripe/Anthropic/Resend US-based, so transfers outside the EEA exist and the SCC wording matters; (3) the controller — resolved since Prompt 624: `lib/controller.ts` names Exotictarget, Lda as the one definition, used here and by the Article 14 notice (Prompt 626). `/legal/subprocessors` carries a "draft under legal review" banner tied to the same gate until then.
11. **Article 14 exposure (603 §D, not a page).** 3297 catalog people (names, roles, 1873 LinkedIn URLs, some emails) never registered; `catalog_people.privacy_notice_sent` and `do_not_contact` are 0 on every row — no notice has ever been sent. The highest legal-risk item found on 2026-09-07; needs its own decision (what the notice says, through which channel, and the do-not-contact path).
12. **Account security (Prompt 602) — shipped, two things to exercise live.** Change-password, the owner's admin-reset switch (off by default; the admin triggers a reset and never sees a password; "this wasn't me" ends every session), and close-account (owner only, 30-day retention, `/closed`, reopen from Startups inside the window) are in production. Not exercised live: the emails (Resend in production) and a real closure with a Stripe subscription (0 subscriptions today).
13. **Commitment 4's own promise is stale (found while applying Prompt 604, 2026-09-12).** 604's literal text for "We don't read your documents out of curiosity" ends "…it's recorded with the reason and the time, where you can see it too" — true when 604 was written (`/documents/access-log` showed a "Sherlock team access" section built from `viewer_enter`/`viewer_exit`). Prompt 886/877 (2026-09-10, Nuno's decision, repeated from 877) reversed that: an admin's access to a workspace is logged internally and is deliberately **not** disclosed to the customer, and that section was removed from the same page. 604 said not to reword its literal text, so the sentence is unchanged and its link is simply omitted (linking to a page that no longer shows what the sentence promises would be worse) — `src/content/commitments/v1.ts`'s own comment on entry 4 and a matching test lock this in. Needs a decision before the gate is ever switched on: soften the sentence, or reconsider the 886/877 non-disclosure for this one narrow case.
