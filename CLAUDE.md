...
- Every new Postgres `view` must specify `with (security_invoker = true)` explicitly in its own `create` statement. A bare `create or replace view` silently clears a `security_invoker` option applied later via `alter view ... set (...)` while preserving any separately-applied `revoke`/`grant` — confirmed empirically (2026-08-06, migration 0135) — so relying on a follow-up `alter` instead of the clause at the point of definition lets the gap silently reopen on the next schema replay. If a view is a deliberate cross-user aggregate that genuinely needs SECURITY DEFINER semantics (e.g. `matchdeal_startup_hype`, which must bypass RLS to sum across users), it must declare `with (security_invoker = false)` explicitly (not omit the clause) with a comment justifying why, AND the same migration must `revoke all ... from public, anon, authenticated` on it — the exception is only safe once PostgREST can no longer serve it, never on the DEFINER behaviour alone (this is exactly the view whose missing revoke caused the 2026-08-06 leak; an exception clause that doesn't also mandate the revoke would have exempted the bug it was meant to prevent).
