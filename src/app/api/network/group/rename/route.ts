// Prompt 317 — My Network 2/9. Owner-only rename.
import { NextResponse } from 'next/server';
import { createClient } from '@supabase/supabase-js';
import { serverClient } from '@/lib/supabase-server';
import { assertNotViewer } from '@/lib/developer-viewer';
import { networkAvailable } from '@/lib/network-capability';
import { resolveActorId, renameGroup } from '@/lib/network-db';
...
export async function POST(req: Request) {
  ...
  const body = await req.json().catch(() => ({})) as { groupId?: string; name?: string };
  const name = body.name?.trim();
  if (!body.groupId || !name) return NextResponse.json({ ok: false, error: 'Missing groupId or name.' }, { status: 400 });

  const result = await renameGroup(admin, body.groupId, actor.actorId, name);
