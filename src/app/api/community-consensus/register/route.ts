...
async function arbitrateEquality(apiKey: string, model: string, field: string, a: string, b: string, catalogId: string): Promise<{ sameValue: boolean; canonicalValue: string | null }> {
  ...
  await logAiCall({ route: '/api/community-consensus/register', purpose: 'field_arbitration', model, usage: data.usage, orgId: null, targetType: 'catalog_entities', targetId: catalogId });
