...
export async function POST(req: Request) {
  ...
  // Prompt 501 — a aplicação em si vive agora em applyInvestorTierToFirm,
  // partilhada com o webhook do Stripe (que aplica a MESMA regra "todos os
  // assentos activos da firma" quando um pagamento entra). O que esta rota
  // acrescenta por cima — auditoria de admin e o email de decisão — continua
  // aqui, porque é dela e não da regra.
  const applied = await applyInvestorTierToFirm(admin, entityId, tier);
  const { members, previousTier } = applied;
  if (applied.error) {
    const status = applied.error === 'No active seats for this firm.' ? 404 : 500;
  ...
  if (resendConfigured) {
    ...
    const body = approved
      ...
      : `We reviewed your plan-change request and are keeping your account on ${TIER_NAME[(previousTier as MatchdealTier | null) ?? (tier as MatchdealTier)]} for now. Reply to this email if you have questions.`;
