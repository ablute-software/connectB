...
const KIND_LABEL: Record<string, string> = { note: 'Nota interna', reply: 'Resposta', status_change: 'Alteração', email_sent: 'Email enviado' };
...
export default function SupportTicketPage() {
  ...
  async function act(body: Record<string, unknown>) {
    ...
    try {
      ...
      if (!data.ok) { setErr(data.error ?? 'Falhou.'); return; }
  ...
  if (!ticket) return <p className="text-sm text-gray-400">A carregar…</p>;

  return (
    <div className="max-w-3xl space-y-4">
      <Link href="/backoffice/support" className="text-xs text-[#0E7490] hover:underline">← Assistência ao Cliente</Link>
      ...
      <Card title={ticket.subject}>
        ...
        {ticket.context && <p className="mt-2 text-xs text-gray-500">Ecrã: {ticket.context}</p>}
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          <label className="text-xs text-gray-500">Prioridade
      ...
      <Card title="Linha temporal">
        {events.length === 0 ? <p className="text-sm text-gray-400">Sem eventos ainda.</p> : (
      ...
      <Card title="Nota interna">
        <textarea value={note} onChange={(e) => setNote(e.target.value)} rows={2}
          placeholder="Só visível no back-office…" className="w-full rounded-lg border border-gray-300 p-2 text-sm" />
      ...
      <Card title="Responder">
        <textarea value={reply} onChange={(e) => setReply(e.target.value)} rows={4}
          placeholder={`Responder a ${ticket.name}…`} className="w-full rounded-lg border border-gray-300 p-2 text-sm" />
        ...
        <button disabled={busy || !reply.trim()} onClick={() => act({ action: 'reply', body: reply, alsoEmail })}
          ...
          {alsoEmail ? 'Responder e enviar email' : 'Guardar resposta'}
        </button>
      </Card>
    </div>
  );
