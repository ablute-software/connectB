...
// Prompt 327 Pedido B — suggestCapitalFixes() itself is untouched (still
// used, still returns the same cases); what changed is where its output
// renders. It used to sit inside this same card, above the real funding
// rows, with nothing but an amber border telling them apart — which is
// exactly why Nuno read "Nuno Marujo has €100,000 of interest… never
...
function PipelineCleanupCard() {
  ...
  return (
    <Card title="Pipeline cleanup">
      <p className="mb-2 text-xs text-gray-500">
        Interest amounts sitting on Pipeline entities that don&apos;t look like real, live interest anymore — not a
        record of investment, just a data-hygiene suggestion for you to confirm or dismiss.
      </p>
...
export function PreviousFundingCard() {
  const { db, addFundingRound, removeFundingRound } = useStore();
          ...
          <ul className="mb-3 space-y-1">
            {rounds.map((r) => (
              <li key={r.id} className="flex items-baseline gap-2 text-sm">
                ...
                {/* Prompt 327 Pedido B — WHO invested, the field that was
                    missing entirely; free text, same as label itself. */}
                {r.investor_name && <span className="text-gray-600">· {r.investor_name}</span>}
          ...
          <button
            ...
            onClick={async () => {
              await addFundingRound({
                ...
                investor_name: investorName.trim() || undefined,
              });
              setInvestorName(''); setLabel(''); setAmount(''); setYear('');
            }}
