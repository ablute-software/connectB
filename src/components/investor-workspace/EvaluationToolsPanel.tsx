...
function WeightBar({ criterion, disabled, onDragEnd, onChange }: {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="w-28 shrink-0 truncate text-sm text-gray-800" title={criterion.label}>{criterion.label}</span>
      <div ref={trackRef}
    ...
    <div className="max-w-lg space-y-4">
      ...
      {criteria.length === 0 ? (
        ...
        <ul className="space-y-2">
          {criteria.map((c, i) => (
            <li key={c.id} className="flex items-center gap-2 rounded-lg border border-gray-200 bg-white p-2.5">
