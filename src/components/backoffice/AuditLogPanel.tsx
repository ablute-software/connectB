'use client';
// Prompt 599 §1 — extracted verbatim from src/app/metrics/page.tsx (Prompt
// 69 Bloco 2's panel) so the back-office's own Audit log page and Metrics
// render ONE implementation, not two that drift. The sidebar's "Audit log"
// entry had no href at all — a placeholder from 576 Fase 1 — while the data
// (viewer_enter/exit, consensus, dossier edits) and this panel both already
// existed. Nothing here is new logic; only the home is.
...
export function AuditLogPanel({ defaultExpanded = false }: { defaultExpanded?: boolean }) {
  ...
  // A page that opens expanded must load on mount — the collapsed default
  // only loads on the first Expand click.
  useEffect(() => {
    if (defaultExpanded && !loadedOnce) { setLoadedOnce(true); load(0, false); }
