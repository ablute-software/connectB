'use client';
// IRM_SPEC §9 — interaction history import. Moved from src/app/import/page.tsx
// (formerly its own top-level route) into the "Data & imports" separador on
// /settings — logic unchanged, only the export changed from a page default
// to a named panel. Upload → AI extraction → staging review (nothing lands
// unreviewed) → commit (reconciliation + post-import analysis). Generic
// version: only .txt/.csv parse today (see DECISIONS.md); the extraction
// shape is intentionally loose (jsonb) until real example files arrive to
// tune the field mapping.
//
// The links below to /import/structured and /import/md stay real routes,
// not separadores — they're alternate import flows with their own multi-step
// UI, not a merge candidate.
