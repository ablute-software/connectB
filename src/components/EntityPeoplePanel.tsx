    ...
    <Card title="Team">
      {showKeyPeopleFallback ? (
        <div>
          ...
          <p className="mb-2 text-xs text-gray-400">From submitted research — verified, not yet a live catalog match.</p>
              ...
              return (
                <li key={`${p.fullName}-${i}`} className="flex flex-wrap items-center justify-between gap-2 py-2">
                  ...
                  {already ? (
                    ...
                    <button
                      onClick={() => addPerson({ entity_id: entityId, full_name: p.fullName, role: p.role ?? undefined })}
