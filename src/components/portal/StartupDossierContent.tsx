...
// Prompt 681 §3 — the investor Pipeline's sliding panel embeds this SAME
// component (never a fork) instead of the page it's normally reached at.
// `orgId`/`variant`/`onClose` are the only seams that exist for that:
//   - orgId: falls back to the route's own useParams() when absent, so the
//     standalone page (no props at all) is byte-for-byte unchanged.
//   - variant 'panel': swaps min-h-screen for a flex column that fills
//     whatever fixed-height container the panel gives it (its own
//     scrollbar via overflow-y-auto on this same div, so the sticky header
//     below still pins correctly — sticky just needs a scrolling ancestor,
//     and this div now is one); hides the Track & Evaluate toggle (that
//     mode is a 3-column layout that doesn't fit a ~340px-wide panel, per
//     the prompt's own instruction); adds "Open full dossier ↗".
//   - onClose: replaces the two hardcoded "← Back to Pipeline" links (which
//     assume this IS the whole page) with a callback the panel supplies.
// Deliberately NOT parameterized: the tab query-param scheme. The
// standalone route keeps writing ?tab= via the router exactly as before;
// inside the panel, tab selection is local-only state (see `tab`/`setTab`
// below) rather than a second, colliding ?tab= on /portal's own URL (which
// already uses that name for the outer shell's own tab switcher). The
// panel's OWN address (?startup=<orgId>, in PipelinePanel.tsx) is what
// satisfies "shareable, reloadable" — which sub-tab was open inside it was
// never part of that requirement.
export interface StartupDossierEmbedProps { orgId?: string; variant?: 'page' | 'panel'; onClose?: () => void }

export function StartupDossierPageInner({ orgId: orgIdProp, variant = 'page', onClose }: StartupDossierEmbedProps = {}) {
  const router = useRouter();
  const pathname = usePathname();
  const params = useParams<{ orgId: string }>();
  const orgId = orgIdProp ?? params.orgId;
  const [panelTab, setPanelTab] = useState<'overview' | 'documents' | 'messages' | 'activity' | 'market'>('overview');
  ...
  const tab: 'overview' | 'documents' | 'messages' | 'activity' | 'market' = variant === 'panel'
    ? panelTab
    : (tabParam === 'documents' || tabParam === 'messages' || tabParam === 'activity' || tabParam === 'market' ? tabParam : 'overview');
  ...
  //
  // Prompt 681 §3 — that guarantee is specific to this page owning ?tab= on
  // its OWN route; inside the sliding panel this component is mounted on
  // /portal instead, where ?tab= already means something else (the outer
  // shell's own tab switcher), so panel mode keeps this local instead of
  // writing a colliding query param.
  const setTab = useCallback((next: 'overview' | 'documents' | 'messages' | 'activity' | 'market') => {
    if (variant === 'panel') { setPanelTab(next); return; }
  ...
  }, [router, pathname, searchParams, variant]);
  ...
  // variant !== 'panel' guard: a remembered trackEvaluate=true preference
  // (fetched unconditionally below) must never turn on the 3-column layout
  // inside the ~340px panel — that mode is exactly what "Open full dossier"
  // exists to reach instead.
  const showRightColumn = variant !== 'panel' && trackEvaluate && (tab === 'overview' || tab === 'documents');
    ...
    return (
      <div className="mx-auto mt-16 max-w-sm rounded-lg border border-gray-200 bg-white p-6 text-center">
        ...
        {variant === 'panel' && onClose ? (
          <button onClick={onClose} className="mt-3 text-xs text-[#0E7490] hover:underline">← Back to Pipeline</button>
        ) : (
  ...
  return (
    <div className={variant === 'panel' ? 'flex h-full min-h-0 flex-col overflow-y-auto bg-[#F7F9FA]' : 'min-h-screen bg-[#F7F9FA]'}>
      <div className="sticky top-0 z-10 border-b border-gray-100 bg-white/95 px-4 py-3 backdrop-blur md:px-8">
        <div className="flex items-center justify-between gap-2">
          {variant === 'panel' && onClose ? (
            <button onClick={onClose} className="text-xs text-gray-400 hover:underline">← Back to Pipeline</button>
          ) : (
            <Link href="/portal" className="text-xs text-gray-400 hover:underline">← Back to Pipeline</Link>
          )}
          {/* Prompt 681 §3 — the panel's one-way door out to the full page:
              Track & Evaluate's 3-column layout doesn't fit here. */}
          {variant === 'panel' && (
            <Link href={`/portal/startup/${orgId}`} className="text-xs font-medium text-[#0E7490] hover:underline">
              Open full dossier ↗
            </Link>
          )}
        </div>
