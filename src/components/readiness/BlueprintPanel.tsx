...
const SEVERITY_STYLE: Record<string, string> = {
  critical: 'bg-red-100 text-red-800', high: 'bg-amber-100 text-amber-800', medium: 'bg-gray-100 text-gray-600',
};
...
export function BlueprintPanel() {
  ...
  async function submitAnswer(dismissed: boolean) {
    if (!gap) return;
    setBusy(true); setError(null);
    try {
      const res = await fetch('/api/blueprint/answer', {
        ...
        body: JSON.stringify({
          gapKey: gap.key, rule: gap.rule, option: option || undefined, answer: answer || undefined,
          analysisId: state?.analysis?.id, dismissed,
        }),
      });
      const body = await res.json().catch(() => ({}));
  ...
  return (
    <div className="space-y-4">
      ...
      {/* §3 — o interrogatório: uma pergunta de cada vez. */}
      {gap && (
        <Card title={<span className="text-[#0E7490]">What&apos;s missing ({state.gaps.length} left)</span>}>
          ...
          <div className="mt-2 flex items-center gap-2">
            <button onClick={() => submitAnswer(false)} disabled={busy || (!option && !answer.trim())}
              className="rounded-lg bg-[#0E7490] px-3 py-1.5 text-xs font-medium text-white disabled:opacity-40">
              Save answer
            </button>
            <button onClick={() => submitAnswer(true)} disabled={busy}
          ...
          <p className="mt-1.5 text-[11px] text-gray-400">
            Your answer becomes a claim in your own words. Its strength is measured from what you write — never chosen.
          </p>
        </Card>
      )}
