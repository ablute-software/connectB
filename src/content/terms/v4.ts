// Prompt 742 §C.1 — v3's text verbatim (not one other word changed) except
// for the version line and one new sub-clause: **4.6, How investor
// activity is used** — the contractual basis for Part D's own new
// behaviour (documents now open inside the Platform, not as a bare
// redirect, so time-on-document and pages-seen are recorded; Prompt 741's
// signal ledger already records the coarser access-level steps this
// clause also covers). Its text is Nuno's own, approved in chat on
// 26/09/2026 — verbatim, not reworded; if he changes it before merge, his
// later version replaces this one, also verbatim: Claude Code does not
// edit legal text.
// The version line in the document header moves 3.0 → 4.0 (same pattern as
// the v2 → v3 and v1 → v2 bumps). Nothing else in the text differs.
export const TERMS_V4_MARKDOWN = `# Sherlock Deal — Terms & Conditions of Use
...
4.6. **How investor activity is used.** When an Investor User interacts with a startup on the Platform — for example, opening its profile, asking for a further disclosure level, or opening a document the founder has shared — the Platform records that step (what, when, and at which access level) and, while a startup's profile or document is open in the Platform, how long it is actively viewed and how many pages are seen. We use this to improve the matches we show you and to keep the disclosure process fair to both sides. Founders can see which of their documents were opened, by whom, for how long and how many pages, and the disclosure levels you reach. They never see your private notes, evaluations or pass reasons, and anything we share with founders beyond that is aggregated so that no individual investor can be identified.
