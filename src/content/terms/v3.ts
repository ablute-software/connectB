// Prompt 514 — v2's text verbatim (not one other word changed) except for
// two sub-clauses of Clause 7.1 (Acceptable use), broadened at Nuno's
// request after the Crunchbase Terms of Service, which prohibit crawling/
// scraping and use-for-training in absolute terms rather than only when
// aimed at a competitor:
//   7.1(d) — was "scrape, harvest, crawl or mass-extract data from the
//     Platform, or use it to build a competing database". Now covers
//     automated mass-extraction of Content or data AND copying/storing any
//     significant portion of it, whether or not a database (competing or
//     otherwise) is the purpose.
//   7.1(i) — was "use the Platform to develop, train or improve a competing
//     product or model". Now covers training, fine-tuning, evaluating or
//     otherwise developing/improving ANY machine-learning or AI model,
//     whether or not competing, and regardless of whether the User is on a
//     paid or free plan.
// The version line in the document header moves 2.0 → 3.0 (same as the
// v1 → v2 bump); nothing else in the text differs. This is the contractual
// half of Prompt 514 — src/app/robots.ts is the technical signal half, and
// only this clause gives a basis to act against a bot that ignores it.
// Same not-a-filesystem-read reasoning as v1.ts for why this is a plain
// exported string.
export const TERMS_V3_MARKDOWN = `# Sherlock Deal — Terms & Conditions of Use
