...
export function nextBestAction(db: Db, entityId: string, now = new Date(), dealMessageTouches: DealMessageTouch[] = []): string | undefined {
  ...
  if (mode === 'parked') {
    ...
    // Prompt 414 §3.2 — reworded from "No reopen trigger recorded — set
    // one" (read as a founder oversight to fix) to an honest statement of
    // what Sherlock itself found: nothing structural. reawakeningProposals
    // can't even apply here (they key on rejection_code_id, which only
    // exists after a formal pass — a genuinely 'parked' entity, by
    // effectiveMode's own definition, never had one; a coded pass makes
    // effectiveMode return 'closed' instead), so there is no signal left
    // to check beyond what the branches above already ruled out. Same
    // spirit as BARS's "Not assessed": absence of signal is stated, never
    // disguised as an unfinished chore.
    return revisit
      ? `Frozen — revisit on ${revisit.slice(0, 10)}. Sherlock hasn't found a structural reason to reopen this yet — set your own note below, or leave it frozen.`
      : "Frozen — no revisit scheduled. Sherlock hasn't found a structural reason to reopen this yet — set your own note below, or leave it frozen.";
  }
  if (mode === 'closed') {
    ...
    // Prompt 414 §3.2 — the prompt names the 'parked' branch's own
    // fallback specifically (above), but this is the structurally
    // identical situation one level up (closed, not parked): the
    // pendingReactivations check above already looked for a cleared
    // rejection_code and found none, same underlying fact this branch's
    // old copy described as "no trigger recorded" without saying Sherlock
    // had already looked. Reworded for the same honesty, deliberately,
    // for consistency between the two parallel fallbacks rather than
    // leaving one honest and one not.
    return `Passed${age ? ` ${age} ago` : ''}${category ? `, over ${category}` : ''}. Sherlock hasn't found a structural reason to reopen this yet — set your own note below, or leave it closed.`;
  }
  ...
  if (summary.nextStep) return followUpTaskDisplayTitle(summary.nextStep, now);
  return undefined;
}
...
// Prompt 414 §1 — a task.title generated by suggestNextAction's OUTBOUND
// branch below ("Wait for a reply until <date> — then <verb>") gets baked
// in once, at save time, and never recomputed — so it keeps saying "wait"
// long after the deadline it names has passed, telling the founder the
// opposite of what to do. This derives the DISPLAY title live, every
// render, without ever rewriting the stored task.title itself.
//
// Discriminant: NOT just kind==='follow_up' + action_type in
// (follow_up_no_reply|follow_up_thread) + due_at, as the prompt's own
// first-pass wording suggested — confirmed by reading RailLogForm.tsx's
// acceptSuggestion(): an INBOUND suggestion (e.g. classification
// 'meeting_request' -> title "Schedule the meeting", actionType
// follow_up_thread) is saved through the exact same
// {kind:'follow_up', action_type, due_at} shape, so those three fields
// alone can't tell an inbound "Schedule the meeting" task from an
// outbound "Wait for a reply..." one — only the OUTBOUND branch's title
// ever has this literal prefix, so matching on it directly is the actual
// precise discriminant, not an invented field.
const FOLLOW_UP_WAIT_PREFIX = 'Wait for a reply until ';

export function followUpTaskDisplayTitle(task: Pick<TaskItem, 'title' | 'due_at' | 'kind'>, now: Date = new Date()): string {
  if (task.kind !== 'follow_up' || !task.due_at || !task.title.startsWith(FOLLOW_UP_WAIT_PREFIX)) return task.title;
  if (now < new Date(task.due_at)) return task.title; // deadline hasn't passed — "wait" still reads correctly
  const rest = task.title.slice(FOLLOW_UP_WAIT_PREFIX.length); // "<date> — then <verb>" (or "<date> — <verb>")
  const dashIdx = rest.indexOf(' — ');
  if (dashIdx === -1) return task.title; // not the exact template shape (e.g. hand-edited) — don't guess
  const dateStr = rest.slice(0, dashIdx);
  const afterDash = rest.slice(dashIdx + 3).replace(/^then\s+/, '');
  return `No reply since ${dateStr} — ${afterDash}`;
}
