...
export async function sendAccessGrantedEmail(admin: SupabaseClient, args: {
  ...
  try {
    ...
    // Prompt 535 — renders through the ONE surviving template module.
    // Prompt 526 Part A introduced a second renderer
    // (src/lib/email-templates/guest-access-email.ts) in parallel with the
    // guest-invite route's own; reconciling the two branches kept
    // src/lib/guest-access-email.ts (the byte-for-byte approved v2 package,
    // multipart, assets already served from /email/guest-access) and deleted
    // this file's original one. What 526 Part A was actually FOR survives
    // untouched, and is why this module still exists: the four non-ad-hoc
    // grant paths (respond, fulfill-upload, access-requests action, manual
    // grant tree) send the same approved template the ad-hoc invite does,
    // instead of the generic shell.
    const email = buildGuestAccessEmail({
      recipientEmail: to,
    ...
    const result = await sendTransactionalEmail({
      ...
      html: email.html,
      // The kept renderer returns a real plain-text part, and resend.ts is
      // multipart — passing it is what stops the text/plain alternative
      // falling back to a stripped copy of the HTML.
      text: email.text,
    });
