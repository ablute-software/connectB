...
type FakeOrg = {
  ...
  // Prompt 563, migration 0311 — on `main` and kept by this landing.
...
describe('eligiblePipelineOrgIds', () => {
  ...
  // production read orgs.moderation_status. Closed by Prompt 571's check,
  // which landed on `main` while 850 was in flight; Nuno kept 571's strict
  // form over 850's isVisibleToOthers on 07/09, so a suspension lifts only
  // on an explicit undo — never on a time-box expiring.
  it('excludes a back-office suspended or deleted org, and restores it only on undo', async () => {
    ...
    const undone = fakeAdmin([completeOrg('org-back', { moderation_status: 'active' })], []);
    expect(await eligiblePipelineOrgIds(undone, false)).toEqual(['org-back']);
  });
