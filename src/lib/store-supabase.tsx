...
async function loadAll(sb: SB, orgId: string): Promise<Db> {
  const [
    ...
    // Prompt 852 §A — startup_investor_decisions (0340). Same missing-table-
  ...
  return {
    ...
    // Prompt 852 §A — migration 0340. Reverted rows are loaded too: the
...
export function SupabaseStoreProvider({ children }: { children: React.ReactNode }) {
  ...
  const api = useMemo<StoreApi>(() => ({
    ...
    updateDocumentsVisibility(ids, visibility) {
      ...
      const prev = dbRef.current;
      commit({ ...prev, documents: prev.documents.map((d) => idSet.has(d.id) ? { ...d, visibility } : d) });
    ...
    logSystemNote(entityId: string, content: string) {
      ...
      const o = orgIdRef.current;
      if (o) persist(sb.from('interactions').insert({ ...note, org_id: o }), 'logSystemNote');
    },
